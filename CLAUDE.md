...
- **Never state prices, cohort dates, or waitlist status on this site.** The LMS at `https://university.pravinmishra.com` (Payhip) is the single point of sale for all three DMI tracks (Foundation, Self-Paced Engineer, Live Internship) — those facts live only there and have gone stale here before (e.g. hardcoded cohort start dates in the hero/CTA). Link to University, don't restate it.
  - **One deliberate exception (Pravin's call, 2026-09-24): the next-cohort CTA.** "Join Cohort 4: starts 9 Jan 2027" appears on the leaderboards, home, How it works, Courses, Roadmaps, the proof pages (graduates/champion/alumni/stories), the end of every blog post, and under the nav of every `s/` badge page. The label, link and start date live **only** in `cohort-cta.js`. Pages carry hooks (`data-cohort-cta`, `data-cohort-cta="href"`, `data-cohort-show hidden`) and the script fills them in, then stops once `startsAt` passes: new buttons stay hidden and repurposed links fall back to what's in the HTML. The popup (`cohort-popup.js`) has its own copy of the date; for the next cohort, edit both files and nothing else. Badge pages get the hooks from `dmi-review-agent`'s `review_agent/badges.py`.
...
- **`apply.html`, `enroll.html` and `join.html` are deliberately excluded** —
  they're distraction-free conversion landing pages that carry **no nav and no
  footer at all**, so the only way off the page is the form. Don't "fix" this by
  adding the shared chrome back; `tools/build-nav.py` skips them by name.
